...
class state_est_task:
    def __init__(self, sL, sR, voltageL, voltageR, yaw, yaw_rate, shareEstimator, estS, estPsi, estOmegaL, estOmegaR, dataValsArc, dataValsPsi):
        self.sL : Share = sL
        self.sR : Share = sR
        ...
        self.dataValsArc : Queue = dataValsArc
        self.dataValsPsi : Queue = dataValsPsi
        self._state = S0_WAIT
    ...
    def run(self):
        while(True):
            if self._state == S0_WAIT:
                ...
                u_star = np.array([[self.voltageL.get()],[self.voltageR.get()],[self.sL.get()],[self.sR.get()],[self.yaw.get()],[self.yaw_rate.get()]], dtype = np.float)
                xhat_new = np.dot(self._AD, self._xhat)+np.dot(self._BD, u_star)
                yhat = np.dot(self._CD, self._xhat) + np.dot(self._DD, u_star)
                self._xhat = xhat_new
